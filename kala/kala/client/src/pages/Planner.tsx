...
import { PANCHANG_TO_TASK_MODE, PRIORITY_EXCLAIM, MODE_OKLCH, MODE_TINT, MODE_CARD_BG, MODE_SOLID, MODE_DARK, TASK_MODES, MODE_RGBA } from "../../../shared/types";
...
const MODE_DOT: Record<string, string> = {
  ...
  Activate:   "oklch(0.72 0.16 145)",
  ACTION:     "oklch(0.72 0.16 145)",
  BUILD:      "oklch(0.65 0.08 85)",
  RESTRAINT:  "oklch(0.65 0.12 15)",
  "SELECTIVE ACTION": "oklch(0.72 0.10 200)",
};
...
export default function Planner() {
  ...
  return (
    <div className="min-h-screen w-full relative">
      ...
      {/* ── HERO CARD — selected day, immersive mode gradient (mirrors Today page) ── */}
      {selectedPanchang ? (
        <div className="space-y-4 relative z-10">
          ...
          >
            ...
            >
              {selectedPanchang.dayOfWeek}, {selectedPanchang.date}
            </span>
          ...
          {/* Time Lord Movement — collapsible, same pattern as Today page */}
          <div
      ...
      {/* Calendar */}
      {/* Calendar card — visual anchor with mode-color strip header + border */}
      <div
      ...
      >
        ...
        <div className="grid grid-cols-7 gap-y-1">
          {calendarCells.map((day, idx) => {
            ...
            // Today highlight uses the day's mode color so it doesn't look like Build mode
            const todayModeColor = panchang ? MODE_DOT[panchang.mode] : "var(--color-foreground)";
            ...
            return (
              <button
                ...
                style={{
                  background: isSelected
                    ? withAlpha(todayModeColor, 0.15)
                    : isToday
                    ? withAlpha(todayModeColor, 0.12)
                    : "transparent",
                  border: isSelected
                    ? `1px solid ${withAlpha(todayModeColor, 0.50)}`
                    : isToday
                    ? `1px solid ${withAlpha(todayModeColor, 0.35)}`
              ...
              >
                ...
                {modeColor && (
                  <span className="w-1.5 h-1.5 rounded-full mt-0.5" style={{ background: modeColor }} />
                )}
              </button>
            );
          })}
          ...
          <div className="space-y-2">
            {dueTasks.map((task) => (
              <button
                ...
                style={{ background: MODE_CARD_BG[task.mode as TaskMode] ?? PLANNER_MODE_TINT[task.mode as TaskMode] ?? "var(--color-card)", border: "none" }}
              >
                <span

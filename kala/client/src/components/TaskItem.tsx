...
import { PRIORITY_EXCLAIM, MODE_OKLCH, MODE_RGBA, MODE_TINT, MODE_CARD_BG, MODE_SOLID, type TaskMode } from "../../../shared/types";
...
export default function TaskItem({ task, onToggleComplete, onTogglePin, onDelete, onEdit, onExpandChange, taskModeColor, dayMode }: TaskItemProps) {
  ...
  // Card background: use MODE_CARD_BG (primary hex at 70% opacity) for all task cards
  const cardBg = MODE_CARD_BG[task.mode as TaskMode] || MODE_CARD_BG.Action;
  const modeAccent = taskModeColor || MODE_OKLCH[task.mode as TaskMode];
  const taskModeRgba = MODE_RGBA[task.mode as TaskMode] ?? MODE_RGBA.Action;
      ...
      >
        ...
        >
          <span
            ...
            style={{ color: task.isCompleted ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.95)", fontFamily: "'Inter', ui-sans-serif, sans-serif" }}
          >
            {task.title}
          </span>
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            {task.dueDate && !task.isCompleted && (
              <span
                ...
                style={{
                  background: overdue ? "rgba(0,0,0,0.35)" : "rgba(255,255,255,0.22)",
        ...
        {task.priority && task.priority !== 'Low' ? (
          <span
            ...
            style={{ color: "rgba(255,255,255,0.9)", opacity: task.isCompleted ? 0.4 : 0.9, letterSpacing: "-0.02em" }}
          >
          ...
          <span
            className="text-[9px] font-bold tracking-tight flex-shrink-0"
            style={{ color: "rgba(255,255,255,0.55)", opacity: task.isCompleted ? 0.3 : 0.55, letterSpacing: "-0.02em" }}
          >
            !
          </span>
        ) : null}
        ...
        {/* Pin indicator */}
        {task.isPinned && (
          <Pin size={12} style={{ color: MODE_OKLCH[task.mode as TaskMode], flexShrink: 0 }} />
        )}
          ...
          {addingSubtask ? (
            <div className="px-3 pb-2 flex items-center gap-2">
              <div
                ...
                style={{ borderColor: "rgba(255,255,255,0.40)", background: "transparent" }}
              />
              <input
                ref={addInputRef}
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                style={{
                  color: "rgba(255,255,255,0.9)",
                  background: task.isPinned ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.18)",
                  border: "1px solid rgba(255,255,255,0.25)",
                }}
              >
                <Pin size={10} />

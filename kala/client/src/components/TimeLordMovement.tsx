...
export function TimeLordMovement({ selectedDate, variant = "default", accentColor, darkColor }: TimeLordMovementProps) {
  ...
  const labelColor = immersive
    ? (darkColor ?? accentColor ?? "rgba(255,255,255,0.9)")
    : dayLabelColor;
  const textColor = immersive ? "rgba(255,255,255,0.95)" : "var(--foreground)";
  const mutedColor = immersive ? "rgba(255,255,255,0.78)" : "var(--muted-foreground)";
  const dotColor = immersive ? "rgba(255,255,255,0.5)" : "var(--muted-foreground)";

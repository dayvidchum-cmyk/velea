/**
 * EXTEND CIRCLE ENUM: + 'children', 'pets', 'mentors', 'mentees', 'followers', 'enemies' (David 2026-07-16).
 ...
 *   enemies  → the 6th (shatru — rivals, contests), riding the 6th's rooms
 *
...
async function run() {
  ...
  if (["'children'", "'pets'", "'mentors'", "'mentees'", "'followers'", "'enemies'"].every((v) => type.includes(v))) {
    console.log("· all four already in the enum — skipped"); process.exit(0);
  }
  await db.execute(sql`
    ALTER TABLE tasks MODIFY COLUMN circle
    ENUM('life_partner','children','family','best_friends','inner_circle','friends','mentors','mentees','coworkers','clients','followers','pets','enemies','self','everyone_else') NULL
  `);
  console.log("✅ tasks.circle now includes children, pets, mentors, mentees, followers, enemies — all existing values and data preserved.");
  process.exit(0);
}

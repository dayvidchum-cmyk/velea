...
export async function deleteProfectionYearsForProfile(profileId: number): Promise<void> {
  const db = await getDb();
  if (!db) return;
  await db.delete(profectionYears).where(eq(profectionYears.profileId, profileId));
}

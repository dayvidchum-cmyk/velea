...
async function buildNarrativeInputUncached(profileId: number, dateStr: string, moment: { nowMs?: number; lat?: number; lon?: number; slowOnly?: boolean; dayLoc: { lat: number; lon: number; utcOffset: number }; lifeArea?: LifeAreaKey; areaFocus?: { key: string; label: string; houses: number[]; karaka: string; blurb: string }; eclipseArc?: boolean; mercuryRxArc?: boolean; rxArcPlanet?: "venus" | "mars" | "jupiter" | "saturn"; monthArc?: boolean; dayNudges?: boolean }) {
  ...
  // TEMPORAL SCOPE (the input-prune, David 2026-07-24): a period arc reads a multi-WEEK span; it must
  // NOT inherit the DAY layer — the day-mode/day-star (panchang), the crown (natal.personalApex), the
  // personal day-filter — that is a shorter time horizon leaking into a longer one ("today's gentleness"
  // inside a season read). Strip it for every arc surface. The day read (no arc flag) is untouched.
  // Non-mutating (destructure-omit) so the shared natal object is never corrupted for other callers.
  const isPeriodArc = !!(eclipseSeasonArc || mercuryRxArc || planetRxArc || monthArc);
  const natalOut = isPeriodArc && natal ? (({ personalApex, ...rest }) => rest)(natal as any) : natal;
  return { subject: { profileId: p.id }, date: dateStr, natal: natalOut, natalRetrogradeCount, profection, dasha, transits, ...(!isPeriodArc ? { panchang } : {}), recentReads, humanTime, timeLordTransit, arc, ...(stage ? { stage } : {}), ...(natalCondition ? { natalCondition } : {}), ...(vocation ? { vocation } : {}), ...(dayFilterBlock && !isPeriodArc ? { dayFilter: dayFilterBlock } : {}), ...(meridianAxis ? { meridianAxis } : {}), ...(nodalAxis ? { nodalAxis } : {}), ...(knots ? { knots } : {}), ...(lineage ? { lineage } : {}), ...(openWindows ? { openWindows } : {}), ...(reading ? { reading } : {}), ...(mercuryRx ? { mercuryRx } : {}), ...(moonBright ? { moonBrightness: moonBright } : {}), ...(lifeAreaLens ? { lifeAreaLens } : {}), ...(transitPrecision ? { transitPrecision } : {}), ...(eclipseSeasonArc ? { eclipseSeasonArc } : {}), ...(orientation ? { orientation } : {}), ...(mercuryRxArc ? { mercuryRxArc } : {}), ...(planetRxArc ? { planetRxArc } : {}), ...(monthArc ? { monthArc } : {}) };
}

...
/**
 * The ONE ranked solar year (birthday → birthday) for a user's ACTIVE profile — the single source
 * behind the month calendar's marks AND the /year overview, so they can never tell two stories
 * (David: "one calendar", "i want the marks to reflect the calendar").
 * yearOffset: 0 = the solar year containing today, ±1 = neighbours. Cached in-memory.
 *
 * The implementation moved to vedic/ranked-year.ts (2026-07-20) so the READING can reach the same
 * answer instead of computing its own crown — that split is what let the two crown definitions
 * drift apart (v778). This is now only the active-profile lookup.
 */
async function rankedSolarYearFor(userId: number, yearOffset: number): Promise<any | null> {
  ...
  const user = await getU(userId);
  return rankedSolarYearForProfile(profile, user, yearOffset);
}

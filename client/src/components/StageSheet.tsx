                  ...
                  {[heroes[heroIdx]].map((h, _i) => {
                    ...
                    <button key={i} onClick={() => setSkyIdx(i)}
                      ...
                      <div style={{ position: "relative", zIndex: 1, marginTop: "clamp(90px, 22vh, 200px)", padding: "2.6rem 1.4rem 1.6rem", background: TEXT_SCRIM, animation: "velea-rise 0.7s ease both" }}>
                        ...
                        <p style={{ margin: "0.2rem 0 0", fontSize: "1.85rem", fontWeight: 800, color: "#FDFDFD", fontFamily: "var(--font-title)", textShadow: ts, lineHeight: 1.05 }}>{h.title}</p>
      ...
      {skyIdx >= 0 && heroes[skyIdx] && (
        <div onClick={() => setSkyIdx(-1)} style={{ position: "fixed", inset: 0, zIndex: 10000, background: "#05060a", cursor: "pointer" }}>
          ...
          <div style={{ position: "absolute", bottom: "calc(env(safe-area-inset-bottom,0px) + 1.6rem)", left: 0, right: 0, padding: "0 1.4rem", pointerEvents: "none" }}>
            ...
            <p style={{ margin: "0.3rem 0 0", fontSize: "1.9rem", fontWeight: 800, color: "#FDFDFD", fontFamily: "var(--font-title)", textShadow: "0 2px 16px rgba(0,0,0,0.6)" }}>{heroes[skyIdx].title}</p>

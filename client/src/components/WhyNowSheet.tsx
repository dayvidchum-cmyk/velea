    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" style={{ background: "var(--dialog-overlay)" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md"
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <p style={{ fontSize: "0.62rem", fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: inkOf(modeColor), margin: 0 }}>Why now?</p>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem", marginTop: "0.85rem" }}>
          ...
          {reasons.map((r) => (
            <span key={r} style={{ fontSize: "0.84rem", fontWeight: 600, padding: "0.35rem 0.75rem", borderRadius: 999, background: `color-mix(in srgb, ${modeColor} 12%, var(--color-card))`, color: inkOf(modeColor, 4.5, 12), border: `1px solid color-mix(in srgb, ${modeColor} 30%, transparent)` }}>{r}</span>
          ))}
